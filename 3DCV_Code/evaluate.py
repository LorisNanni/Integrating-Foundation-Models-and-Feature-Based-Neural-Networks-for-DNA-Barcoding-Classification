...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate 3D-CGR ViT model")
    parser.add_argument("--dataset_name", type=str, required=True, help="Dataset name to use (e.g.: fishes, birds, insect_dataset_genus, 'GNe50000 Filtered', ...)")
    parser.add_argument("--model_path", type=str, default=None, help="Path to the model.pt checkpoint to load for evaluation")
    ...
    parser.add_argument("--batch_size", type=int, default=4, help="Per device test batch size")
...
# #Questions to the user (about whether to do tuning and/or generate images or retrieve them from memory)
...
for fold_index in range(number_of_datasets):
  ...
  learning_rate = 0 ## mock

  batch_size = args.batch_size
  gradient_accumulation_steps = 16 # mock
  ...
  if model_path is None:
    if dataset_name == "beetle":
        # Example of loading from HF Hub if default not overridden and dataset is beetle
        hf_model_name = f"Eddi3k/Vit_beetle_{fold_index}"
        model = ViTLateFusion.from_pretrained(hf_model_name)
        print("Retrieved from memory the model", hf_model_name)
    else:
        model_path = f"trainer_output/Vit_{dataset_name}{simulated_type}{files_identifier}/model.pt"

  if model_path is not None:
    assert os.path.isfile(model_path), f"Model file not found at {model_path}. Please check the path."
    print("loading model from", model_path)
    checkpoint = torch.load(model_path, map_location=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    status = model.load_state_dict(checkpoint["model_state_dict"], strict=True)
    print("model loaded with status", status)
  ...
  logits = outputs.predictions   # shape: (N, num_classes)
  os.makedirs(f"labels_n_logits/{dataset_name}", exist_ok=True)   # Create the directory if it doesn't exist
  np.savetxt(f"labels_n_logits/{dataset_name}/logits_{dataset_name}{files_identifier}.txt", logits)

  print("logits saved at", f"labels_n_logits/{dataset_name}/logits_{dataset_name}{files_identifier}.txt")

  print("Accuracies for all folds:", accuracies)
  print("Mean accuracy across all folds:", np.mean(accuracies))
